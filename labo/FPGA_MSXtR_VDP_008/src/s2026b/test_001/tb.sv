...
module tb;
	...
	// ------------------------------------------------------------
	reg				sdram_init_busy;
	reg		[7:0]	bus_vdp_rdata;
	reg				bus_vdp_rdata_en;
	reg				bus_vdp_ready;
	...
	s2026b u_s2026b (
		...
		.bus_vdp_cs			( bus_vdp_cs		),
		.bus_vdp_rdata		( bus_vdp_rdata		),
		.bus_vdp_rdata_en	( bus_vdp_rdata_en	),
		.bus_vdp_ready		( bus_vdp_ready		)
	);

	// ============================================================
	//	VDP stub model
	//	  - I/O address 0x98-0x9F (bus_vdp_cs で選択済み)
	//	  - ready までのランダム遅延: 0～10 clk
	//	  - read 時は ready 後さらに 0～10 clk でデータ返却
	// ============================================================
	initial begin
		bus_vdp_ready    = 1'b1;	// 通常時は ready = 1 を維持
		bus_vdp_rdata    = 8'h00;
		bus_vdp_rdata_en = 1'b0;

		forever begin
			// VDP 向けの valid 要求が来るまで待つ
			@(posedge clk iff (bus_vdp_cs && bus_valid));
			begin
				...
				if (cap_write) begin
					...
					bus_vdp_rdata    = $urandom;
					bus_vdp_rdata_en = 1'b1;
					@(posedge clk);
					bus_vdp_rdata_en = 1'b0;
					bus_vdp_ready    = 1'b1;
				end
	...
	task automatic ctrl0_write(
		...
		@(posedge clk iff bus_ctrl0_ready); //#1;
	...
	task automatic ctrl1_write(
		...
		@(posedge clk iff bus_ctrl1_ready); //#1;
